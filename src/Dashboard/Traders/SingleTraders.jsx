...
const SingleTraders = (sData) => {
    const { image, nameOf, location, areaOfFarm, organization, crops_name, _id,DateOfFoundation,state_name} = sData.sData;
    return (
        <div>
            <Link  to={`/contact/${_id}`}>
                <div className='relative md:w-[302px] md:h-[302px] mt-10 hover:bg-[#C4D190] hover:rounded-xl rounded-xl border border-[#C4D190] cursor-pointer hover:delay-50 hover:drop-shadow-xl transition duration-300 ease-in-out'>
                    ...
                    <h2 className='absolute top-0 left-0 translate-x-1 translate-y-2 bg-[#ECDB3F] px-[8px] py-[3px] rounded-full w-[90px] text-black font-semibold'>Since {DateOfFoundation}</h2>
                    </div>
                    <div className='text-[18px] ml-2 mt-[16px]'>
                        <h2 className='font-bold'>{nameOf}</h2>
                        <h2>{location} , {state_name}</h2>
                        <h2>{organization}</h2>
                        <h2>{crops_name}</h2>
                    </div>
